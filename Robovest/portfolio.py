...
def portfolio(score):
    ...
    weights_bl = [round(float(item),1) for item in bl_ef_weights[position1]*100]
    return_bl =round(float(bl_ef_returns[position1])*100,2)
    risk_bl = round(float(bl_ef_risks[position1]),2)

    plt.plot(bl_port_risks,bl_port_returns, 'ro')
    line1, = plt.plot(bl_ef_risks,bl_ef_returns,'b-',label='Efficient Frontier')
